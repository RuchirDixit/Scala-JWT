...
object DatabaseService extends LazyLogging with DatabaseConfig {
  ...
  def saveUser(credentials:User) : String = {
    ...
    if(credentials.email.matches(emailRegexPattern)){
      ...
      {
        val future = collection.insertOne(userToBeAdded).toFuture()
  ...
  def getUsersUsingFilter(email : String) : Future[Seq[Document]] = {
      collection.find(equal("email",email)).projection(excludeId()).toFuture()
  }
